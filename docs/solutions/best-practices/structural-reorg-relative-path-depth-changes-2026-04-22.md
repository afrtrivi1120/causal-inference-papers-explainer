...
## Postscript — 2026-04-22 — R → Python/Jupyter migration

Later the same day, the repo's simulation layer was migrated from R scripts to Jupyter notebooks (`v0-r-era` tag cut at the pre-migration HEAD; `simulation.R` files and `shared/r-setup.R` replaced by `simulation.ipynb` files and a top-level `requirements.txt`; the `r-coding-expert` subagent retired in favor of `simulation-notebook-expert`). Which of this entry's six patterns survived that language change:

- **Pattern 1 (single-commit folder moves + compensating edits).** Still applies unchanged. The notebook migration used the same discipline: each paper's port was one commit that deleted `simulation.R` and added `simulation.ipynb` in the same diff — no intermediate HEAD with a broken `Rscript` command surviving past the point where the new documented invocation (`jupyter nbconvert --execute`) was supposed to work.
- **Pattern 2 (`git mv` preserves history for small edits).** Not applicable to this migration — `git rm` + `git add` of a completely different file format (`.R` → `.ipynb`) is a delete-and-create, not a rename. The `v0-r-era` tag is the preservation mechanism instead. When the content format itself changes, tag-based preservation is what you use; `git mv` only helps for path-only moves with minor content changes.
- **Pattern 3 (sweep every encoded surface).** Directly applied — before landing the final commits, a grep sweep confirmed that `simulation.R`, `Rscript`, `shared/r-setup.R`, `install.packages`, and `r-coding-expert` had zero live references across `CLAUDE.md`, `README.md`, and all four `.claude/agents/*.md` files. Pattern 3 is language-independent.
- **Pattern 4 (amend `docs/solutions/` via postscript).** This very section is pattern 4 in action. The entry itself was not rewritten; this postscript was appended. The 2026-04-21 `rscript-cwd-source-path-resolution` entry was not given a second postscript for this migration — its lesson is entirely R-specific (`commandArgs(--file=)` has no analog in notebooks, which run in kernels with a known CWD) and its existing postscript was sufficient to bound the entry's scope.
- **Pattern 5 (test the literally documented command from the repo root).** Applied: after each notebook port, the exact `jupyter nbconvert --to notebook --execute --inplace <path>` command in the updated `CLAUDE.md` was run verbatim from the repo root before committing. Nothing was tested via `jupyter lab` manual execution, or via a wrapper script — the command the reader sees is the command that was exercised.
- **Pattern 6 (subagent prompts are shell-executable promises).** Carried forward into the new `simulation-notebook-expert` agent, whose instruction set explicitly mandates: the warnings filter comes first (before any pandas-touching import), pip-install cells use `-q` and target specific pinned versions, notebook-commit workflow uses `jupyter nbconvert --execute --inplace`. The agent's shell blocks were copy-pasted into a terminal and verified to run before shipping.

The migration also produced a new, higher-order learning worth flagging for future reference: **when the migration is a language change (R → Python), not just a path-depth change, `git mv` history preservation is replaced by tag-based preservation** (`git tag -a <era-name>` at the pre-migration HEAD). The tag is the durable reference point for the retired stack; downstream contributors know to consult it rather than `git log --follow` across a format boundary.
