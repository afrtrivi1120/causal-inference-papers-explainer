...
2. **Verify the artifacts are safe to commit.**
   ...
   - With `<method>` and `<slug>` substituted for the paper you are committing (e.g. `papers/did/ghanem-santanna-wuthrich-selection-parallel-trends/`), `jupyter nbconvert --to notebook --execute --inplace papers/<method>/<slug>/simulation.ipynb` exits 0 and the notebook contains a rendered truth-vs-estimate comparison — the Monte Carlo summary tibble for MC notebooks, or the per-estimator / per-scenario comparison tibble for single-draw notebooks (the simulation-notebook-expert should have already done this and pasted the summary — spot-check if you're unsure).
